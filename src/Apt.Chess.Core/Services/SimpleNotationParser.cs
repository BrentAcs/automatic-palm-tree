...
public static class SimpleNotationParser
{
   ...
   private static readonly Regex _notationFullPattern = new Regex("^(?<file>[a-h])(?<rank>[1-8])-(?<player>[w|b])-(?<piece>[k|q|r|n|b|p])$", RegexOptions.IgnoreCase);

   public static FileAndRank ParseFileAndRank(string notation)
   {
      var match = _notationFileAndRankPattern.Match(notation);
      if (!match.Success)
         throw new ArgumentException($"{nameof(notation)} with value of '{notation}', does not match pattern: '{nameof(_notationFileAndRankPattern)}'.");
      ...
      return new FileAndRank(file, rank);
   }
   ...
   public static ChessPiece ParsePiece(string notation)
   {
      ...
      if (!match.Success)
         throw new ArgumentException($"{nameof(notation)} with value of '{notation}', does not match pattern: '{nameof(_notationPiecePattern)}'");
   ...
   public static void Parse(string notation, out FileAndRank? fileAndRankOut, out ChessColor? colorOut, out ChessPieceType? pieceTypeOut)
   {
      ...
      if (!match.Success)
         throw new ArgumentException($"{nameof(notation)} with value of '{notation}', does not match pattern: '{nameof(_notationFullPattern)}'");
